...
class VaultService {
  ...
  Future<void> buildVault({
    ...
    await for (final entity in projectDir.list(recursive: true)) {
      ...
      final relative = p.relative(entity.path, from: projectDir.path).replaceAll('\\', '/');
      if (exclude.contains(relative) || relative.endsWith('.tampered')) continue;
  ...
  /// Decrypts [vaultFile] with [password] and extracts its contents into
  /// [targetDir] (created if missing), overwriting any files already there.
  /// Throws [VaultOpenException] if the password is wrong or the vault is
  /// corrupted/tampered — those two cases are indistinguishable by design.
  Future<void> restoreVault({
    required File vaultFile,
    required Directory targetDir,
    required String password,
  }) async {
    final zipBytes = await _decryptVault(vaultFile, password);
  ...
  /// Decrypts and verifies [vaultFile] without writing anything to disk —
  /// useful for a "check the vault is still valid" health check that
  /// doesn't require picking a restore location.
  Future<void> verifyVault({required File vaultFile, required String password}) async {
    await _decryptVault(vaultFile, password);
  }
  ...
  Future<List<File>> _listGenerations(Directory vaultDir, String baseName) async {
    final generations = <File>[];
    if (!await vaultDir.exists()) return generations;
    await for (final entity in vaultDir.list()) {
      final name = p.basename(entity.path);
      if (entity is File && name.startsWith('$baseName.') && name.endsWith('.vault')) {
    ...
    generations.sort((a, b) => p.basename(a.path).compareTo(p.basename(b.path)));
    return generations;
  }

  Future<void> _pruneOldGenerations(Directory vaultDir, String baseName, int retainCount) async {
  ...
  Future<List<int>> _decryptVault(File vaultFile, String password) async {
    ...
    try {
      header = jsonDecode(await vaultFile.readAsString()) as Map<String, dynamic>;
    } catch (_) {
      throw VaultOpenException('Vault file is not readable — it may be corrupted.');
    }
