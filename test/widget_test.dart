...
void main() {
  ...
  testWidgets('App boots to an empty library with a New Project action', (
    tester,
  ) async {
    final container = ProviderContainer(
      overrides: [
        libraryServiceProvider.overrideWithValue(
          LibraryService(rootOverride: tempDir),
        ),
      ],
    );
  ...
  testWidgets('New Project dialog validates title and can be cancelled', (
    tester,
  ) async {
    final container = ProviderContainer(
      overrides: [
  ...
  testWidgets('Settings > Backup & Vault offers first-time setup', (
    tester,
  ) async {
    final container = ProviderContainer(
      overrides: [
    ...
    expect(
      find.widgetWithText(FilledButton, 'Set up vault password'),
      findsOneWidget,
    );
    expect(
      find.textContaining('no way to recover this password'),
      findsOneWidget,
    );
  });
  ...
  testWidgets('Project shell exposes reference panels and opens a character', (
    ...
    final project = (await tester.runAsync(
      () => library.createProject(title: 'Test Novel'),
    ))!;
    final owner = ContentOwner.project(project);
    await tester.runAsync(() async {
      final characters = await container.read(
        characterServiceProvider(owner).future,
      );
      ...
      await container.read(characterListProvider(owner).future);
      await container.read(worldListProvider(owner).future);
      await container.read(storyNoteListProvider(owner).future);
      await container.read(noteFoldersProvider(owner).future);
      await container.read(todoListProvider(owner).future);
    });
    ...
    expect(
      container.read(openReferenceProvider)?.kind,
      ReferenceKind.character,
    );
    expect(
      find.byType(ProfileEditor),
      findsOneWidget,
  ...
  testWidgets(
    'Reference Panel shows quick-reference fields for a mentioned character',
    (tester) async {
      final projectRoot = Directory.systemTemp.createTempSync(
        'narraity_refpanel_test_',
      );
      addTearDown(() => projectRoot.deleteSync(recursive: true));
      final library = LibraryService(rootOverride: projectRoot);
      ...
      container.read(sceneMentionedNamesProvider.notifier).state = [
        'Elena Vance',
      ];
      ...
      expect(
        find.textContaining('No profile named "Nobody At All"'),
        findsOneWidget,
      );
  ...
  testWidgets(
    ...
    'route is popped back to the root first)',
    (tester) async {
      final projectRoot = Directory.systemTemp.createTempSync(
        'narraity_series_open_test_',
      );
      addTearDown(() => projectRoot.deleteSync(recursive: true));
      final library = LibraryService(rootOverride: projectRoot);
      ...
      await tester.runAsync(() async {
        ...
        final loadedOwner = ContentOwner.project(loadedProject);
        await container.read(manuscriptStructureProvider(loadedProject).future);
        await container.read(characterListProvider(loadedOwner).future);
        await container.read(worldListProvider(loadedOwner).future);
        await container.read(storyNoteListProvider(loadedOwner).future);
        await container.read(noteFoldersProvider(loadedOwner).future);
        await container.read(todoListProvider(loadedOwner).future);
      });
      ...
      expect(
        ...
        findsOneWidget,
        reason:
            'should now be looking at the series detail screen, showing its one project',
      );
      ...
      expect(
        ...
        findsNWidgets(5),
        reason:
            'tapping a project inside a series list should open its project shell',
      );
    },
  );
}
