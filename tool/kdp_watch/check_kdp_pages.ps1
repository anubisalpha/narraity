<#
.SYNOPSIS
  Fetches KDP's formatting help pages, diffs each against its last saved snapshot, and files a
  GitHub issue if anything changed. Pure fetch+diff+issue - no LLM judgment involved anywhere in
  the pipeline, including the "flag it" step.

.DESCRIPTION
  Used to keep KDP_CRIBSHEET.md (and PLAN.md's Phase 6.3 section) in sync with Amazon's actual
  formatting rules, since KDP can change trim sizes, margins, page-count ranges, EPUB constraints,
  etc. without notice. Covers paperback/hardcover/eBook formatting plus the Kindle Publishing
  Guidelines pages most relevant to Narraity's EPUB exporter (reflowable text/images/tables,
  navigation, QA standards, accessibility).
...
.NOTES
  ...
  text change won't be flagged, but nav/boilerplate churn on Amazon's side could still produce
...
$pages = @(
    @{ Name = 'paperback';       Url = 'https://kdp.amazon.com/en_US/help/topic/G201834190' }
    @{ Name = 'trim-bleed-margins'; Url = 'https://kdp.amazon.com/en_US/help/topic/GVBQ3CMEQW3W2VL6' }
    @{ Name = 'front-body-back-matter'; Url = 'https://kdp.amazon.com/en_US/help/topic/GDDYZG2C7RVF5N9J' }
    @{ Name = 'hardcover';       Url = 'https://kdp.amazon.com/en_US/help/topic/GKYZRXFBZH2LDXAK' }
    @{ Name = 'ebook';           Url = 'https://kdp.amazon.com/en_US/help/topic/G201723130' }
    @{ Name = 'reflowable-text'; Url = 'https://kdp.amazon.com/help?topicId=GH4DRT75GWWAGBTU' }
    @{ Name = 'reflowable-images'; Url = 'https://kdp.amazon.com/help?topicId=G75V4YX5X8GRGXWV' }
    @{ Name = 'reflowable-tables'; Url = 'https://kdp.amazon.com/help?topicId=GZ8BAXASXKB5JVML' }
    @{ Name = 'navigation-guidelines'; Url = 'https://kdp.amazon.com/help?topicId=GY3AD8C6C6GAG42N' }
    @{ Name = 'qa-standards';    Url = 'https://kdp.amazon.com/help?topicId=GGRXLC5USU4H67YM' }
    @{ Name = 'accessibility-guidelines'; Url = 'https://kdp.amazon.com/help?topicId=GF9Z3HLUMTRJ6QPL' }
)
...
function Get-NormalizedText {
    ...
    # Decode the handful of entities KDP's help pages actually use.
...
if ($changedPages.Count -gt 0) {
    ...
    $issueTitle = "KDP formatting pages changed: $($changedPages -join ', ') ($(Get-Date -Format 'yyyy-MM-dd'))"
    $tick = [char]96
    $issueBody = "Automated check (${tick}tool/kdp_watch/check_kdp_pages.ps1${tick}) detected changes on the following KDP help pages, which feed PLAN.md's Phase 6.3 (KDP export) section:`n`n" + ($issueBodyParts -join "`n")
    $bodyFile = Join-Path $scriptDir '.issue-body.tmp.md'
    Set-Content -Path $bodyFile -Value $issueBody -Encoding utf8

    try {
        & gh issue create --repo $GitHubRepo --title $issueTitle --body-file $bodyFile --label 'kdp-watch' 2>&1 | Write-Host
